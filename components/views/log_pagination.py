...
async def _stringify_logged_events(inter: discord.Interaction, events: list[LoggedEvent]) -> str:
    yaml_events = []
    for event in events:
        user = await inter.client.fetch_user(event.user_id) if event.user_id else None
        event_info = dict(
            id=event.id,
        ...
        event_str = as_code_block(yaml.safe_dump(event_info, sort_keys=False), "yaml")
        yaml_events.append(event_str)
    return "\n".join(yaml_events)
...
class LogPaginationView(discord.ui.View):
    ...
    async def respond_with_view(self, inter: discord.Interaction, ephemeral: bool, start_datetime: datetime | None = None) -> None:
        ...
        if entries:
            self.message_text = await _stringify_logged_events(inter, entries)
        else:
